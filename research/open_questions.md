...
Answer: Still unresolved from this cycle. The completed large run is concentrated on DeepSeek-R1 distill 1.5B over GSM8K, so it identifies a real boundary for that model-benchmark pair but cannot yet decompose family effects from benchmark effects. A comparable Qwen, Llama, or larger DeepSeek follow-up is still needed before attributing the boundary to model family rather than task distribution.
