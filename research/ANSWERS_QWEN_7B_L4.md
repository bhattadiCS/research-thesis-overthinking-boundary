...
Within the current Qwen2.5 instruct 7B L4 run, the most stable currently supported observable is self-reported confidence (confidence, coeff=0.714), while the strongest corruption-side signal is verbosity-confidence proxy (verbose_confidence_proxy, coeff=0.622). True cross-family stability is still unsettled until another family is run at comparable scale, but the current run cleanly identifies the leading signals for this model.
