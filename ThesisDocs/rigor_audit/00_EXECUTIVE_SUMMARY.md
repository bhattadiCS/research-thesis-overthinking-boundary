...
> **Update 2026-07-10 (second) — autonomous fix-everything pass executed.** All CPU-side levers and protocols are done and committed: hunk ruling H13 (`34d76c0`); pipeline hardening H1/H2/H4/H5/H11/H12 (`ce0167c`, grader suite 30/30, patched regrade dry-run = 0 flips); docs/banners/relocations H6–H10/H14 + regenerated MATH aggregate (`07a9573` — **finding: on full data `qwen2p5_7b__math`'s boundary is step 4/weakened, not step 5; the MATH late-boundary story is now 32B-only-clear**). Pre-registered offline experiments ([05_offline_experiment_results.md](05_offline_experiment_results.md)): **P1 empty-answer guard FAILED** (net −116.5; 118 win→loss) — not shipped; **P2 floor DEFENDED** (T_MIN=3 costs −1,912; step-2 stops are 91.9% wins); **P3a blanket token-cap guard FAILED** (−8,034); **P13 temperature monotonicity confirmed 4/4**; **P6 extraction audit**: 0.188% delta, clean for 11 models, 30 DeepSeek-scoped rows pending fallback-semantics replication; **P9 λ-sweep PASSED** (win rate 88.84–90.86% across λ∈[0.02,0.10] vs the ±5pp bar; exact analytic sweep, validated 0 mismatches at λ=0.05); **P3b/P3c FAILED their pre-registrations; P3d per-cell calibration PASSED (+1,546 OOF utility)** — see [05_offline_experiment_results.md](05_offline_experiment_results.md). **P3d is now productized**: `research/outputs/experiment_matrix/_aggregate/calibrated_cell_thresholds.csv` (52 cells, 0 stop mismatches, Σ oof_dU = +1,545.85 exactly matching the census) with generator `research/calibrate_cell_thresholds.py` and leakage recipe + findings in [06_calibrated_thresholds.md](06_calibrated_thresholds.md) — notably, the 7 cells that calibrate toward *more* patience are almost exactly the known late-boundary cells, independently rediscovering the overthinking-boundary structure. Net: the deterministic guards are dead, the floor is defended, and the live levers are the conditional arms P3b–P3d + P4b (GPU).
>
